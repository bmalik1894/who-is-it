...
class GameActor(code:String,manager: ActorRef, host:ActorRef, hostName: String) extends Actor {
    ...
    def receive = {
        ...
        case NextR() => println("NewRoundRequest "+ready)
                        if(questionActive != true){ready += 1}
                        if(ready == players.length){
                            println("SO TRUE" + ready)
                            playRound()
                        }
        ...
        case GimmeQuestion() => println("Gimme")
                                playRound()
    ...
    def playRound(): Unit = {
        println("PLAYING ROUND q length "+ questions.length )
